...
- In poolboy workers have to implement a worker behavior (`poolboy_worker`) which exposes a `start_link/1` function used to start workers. In contrast, gruff (and also pooler) starts workers via an `{M, F, A}` triple.
